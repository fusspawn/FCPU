...
namespace FCPU.Instructions
{
        ...
        : FCPUInstruction
    {
        public JLZ() {
            ...
            ArgCount = 1;
            InsName = "JLZ";
            DocString = "JLZ: jump to label if @r0 < 0";
        }
        ...
        public override void Execute(FCPUState State)
        {
            ...
            if(State.GetRegisterValue(0) < 0)
               State.IP = Arg.Value; // Minus 2 because the ip will auto increment by 2 after executing this instruction
        }            
        ...
        public override void Parse(FCPUState State, string[] Args)
        {
            ...
            State.IP += 1;         
            State.Memory[State.IP] = new FObject(false, State.JumpTable[Args[0]], State, State.IP, false);
            State.IP += 1;
        }
